...
def train(model, batch_size, dataloader, architecture, optimizer_type, device, models_dir):
	"""
	...
	if architecture == "vgg19":
		...
		num_epochs = 300
		lr_anneal_epochs = [args.milestone[0], args.milestone[1], args.milestone[2]]
	else:
	...
	wandb.init(entity="67Samuel", project='Varungohli Lottery Ticket', name=f"Train {architecture}", config={'batch size':batch_size, 'lr':optimizer.param_groups[0]['lr'], 'epochs':num_epochs})
...
if __name__ == '__main__':
	...
	train(model, args.batch_size, dataloader, args.architecture, args.optimizer, device, args.model_saving_path)
